...
export type BacklogResolutionStruct = {
    id: number
    name: string
}

export class BacklogResolution {
    ...
    public async getAll(): Promise<BacklogResolutionStruct[]> {
        const connection = await this._authentication.getConnection()
        if (!connection) return []

        return invoke<BacklogResolutionStruct[]>('backlog_resolution_list', {
